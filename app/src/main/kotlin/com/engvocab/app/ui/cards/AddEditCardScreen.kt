...
fun AddEditCardScreen(cardId: Long?, onDone: () -> Unit, onBack: () -> Unit) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(if (uiState.isNew) "New card" else "Edit card") },
                navigationIcon = {
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                label = { Text("Italian translation") },
                modifier = Modifier.fillMaxWidth(),
            )
...
private fun CardTypeDropdown(selected: CardType, onSelected: (CardType) -> Unit) {
    ...
    val labels = mapOf(
        CardType.WORD to "Word",
        CardType.PHRASAL_VERB to "Phrasal verb",
        CardType.IDIOM to "Idiom",
        CardType.EXPRESSION to "Expression",
    )
