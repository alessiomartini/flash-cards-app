...
fun StudyScreen(onFinished: () -> Unit) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(20.dp)) {
        if (uiState.queue.isNotEmpty()) {
            ...
            Text(
                "${uiState.remaining} da ripassare",
        ...
        Box(modifier = Modifier.weight(1f), contentAlignment = Alignment.Center) {
            when {
                uiState.isLoading -> Text("Caricamento…")
...
private fun SessionComplete(onFinished: () -> Unit) {
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Text("Ottimo lavoro!", style = MaterialTheme.typography.titleLarge)
        Spacer(Modifier.height(8.dp))
        Text(
            "Hai finito i ripassi in scadenza per ora.",
            style = MaterialTheme.typography.bodyMedium,
            textAlign = TextAlign.Center,
        )
        Spacer(Modifier.height(20.dp))
        Button(onClick = onFinished) { Text("Torna alla home") }
    }
}
...
private fun formatInterval(millis: Long): String {
    ...
    return when {
        ...
        days < 30 -> "${days.roundToInt()} g"
        days < 365 -> "${(days / 30).roundToInt()} mesi"
        else -> "${(days / 365).roundToInt()} anni"
    }
}
